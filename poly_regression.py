# -*- coding: utf-8 -*-
"""
Created on Wed Jul 17 15:11:08 2019
...
for d in range(MAX_POLY):
    ...
    plt.title('All curves for degree = {}'.format(d+1))
    plt.show()
...
plt.plot(degrees, train_score.mean(axis=0), label='train score')
plt.plot(degrees, test_scores.mean(axis=0), label='best complexity')
plt.axvline(x=best_degree, linestyle='--', label='best complexity')